...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const errorMessageField = document.querySelector('.login-form__error-field');
    ...
    submitButton.addEventListener('click', event => {
        ...
        // const isValidTitle = validateEmailField(titleInput, titleErrorFiled);
        // const isValidDescription = validatePasswordFiled(passwordInput, passwordErrorField);
        // if (titleInput) {
        //     //form.classList.add('anim_error');
        //     errorMessageField.classList.remove('hidden');
        //     errorMessageField.innerHTML = 'Publish complete!';
        //     setTimeout(() => form.classList.remove('anim_error'), 1000)
        // } else {
        //     form.classList.add('anim_error');
        //     errorMessageField.classList.remove('hidden');
        //     errorMessageField.innerHTML = 'A-Ah! Check all fields';
        //     setTimeout(() => form.classList.remove('anim_error'), 1000)
        // }
        ...
        [titleInput, descriptionInput, authorNameInput].map(elem => { 
            elem.addEventListener('input', ev => { 
                ...
                // Получаем элементы сообщений об ошибках и успешном вводе
                const titleError = document.getElementById('error');
                const titleSuccess = document.getElementById('success');
        
                // Проверяем наличие значения в поле title
                if (elem === titleInput) {
                    if (elem.value) {
...
// <div id="title-error" class="error hidden">check field</div>
    // <div id="title-success" class="success hidden">publish complete</div>
...
// <script>
//     document.querySelector('author-button-upload').addEventListener('click', function() {
//         let file = document.getElementById('author_photo').files[0];
//         let reader = new FileReader();
